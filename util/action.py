...
class ElementActions:
    ...
    def home_press(self):
        self._send_key_event('KEYCODE_HOME')
    ...
    def input_text(self, locator, value, clear_first=False, click_first=True):
        """输入文本
        Args:
            locator: 定位器
            value: 文本内容
        ...
        self._text_ele(element,value,clear_first, click_first)
    ...
    def _send_key_event(self, arg, num=0):
        ...
        if arg == 'KEYCODE_NUM':
            ...
            self.driver.press_keycode(arg)

    def _text_ele(self,element,value,clear_first=False, click_first=True):
        if click_first:
            element.click()
        if clear_first:
            element.clear()
